...
public class Hashing {
	...
	static String[] array = new String[216555]; // make sure your String array
												// is big enough to hold all the
												// data

	public static void main(String[] args) throws IOException {
		File testFile = new File("./dictionary.txt");
		// this is where the file
		// to be sorted is loaded from
		...
		System.out.println("Load factor: " + (double) 216555 / size);
		in.nextLine();
	...
	public static int getHashKey(String word) {
		...
		// for words, a good strategy is to raise each character to successive
		// powers of the alphabet size
		// assume that the alphabet is ASCII characters - a total of 256
		// possibilities
		// each successive character value should be raised to successive powers
		// of 256
	...
	public static int getDoubleHashKey(String word) {
		...
		// it should return a different number for words which generated the
		// same primary hash key value
	...
	public static void fillLinearProbing() {
		...
		for (int i = 0; i < array.length; i++) {
			...
			if (i % 100 == 0) {
				System.out.println(array[i] + " was placed in slot " + index + " of the hash table after " + collisions
						+ " collisions");
			}
			totalcollisions += collisions;
			// print out the information for the last 1,000 words only,
			// otherwise it takes quite long and gets annoying
		}
	...
	public static int modPow(int number, int power, int modulus) {
		...
		// the trick is to use recursion - keep breaking the problem down into
		// smaller pieces and use the modMult method to join them back together
		if (power == 0)
			return 1;
		else if (power % 2 == 0) {
	...
	public static int modMult(int first, int second, int modulus) {
		...
		// you need to multiply numbers in such a way that the result is
		// consistently moduloed to keep it in the range
		// however you want the algorithm to work quickly - having an addition
		// loop would result in an O(n) algorithm!
		// the trick is to use recursion - keep breaking down the multiplication
		// into smaller pieces and mod each of the pieces individually
		if (second == 0)
			return 0;
		else if (second % 2 == 0) {
